---
description: Frontend development standards, best practices, and conventions for React applications including component patterns, state management, UI/UX guidelines, and testing practices
...
This document outlines the best practices, conventions, and standards for frontend applications. These practices ensure code consistency, maintainability, and optimal development experience.
...
- **Build Tool**: Modern build tooling (Vite, Create React App, etc.)
- **React Router**: Client-side routing and navigation

### UI Framework
- **CSS Framework**: Modern CSS framework for responsive design (Bootstrap, Tailwind, Material-UI, etc.)
- **Component Library**: Framework-specific component library
- **Icon Library**: Icon library for UI elements
- **Additional UI Libraries**: Date pickers, modals, tooltips, etc. as needed

### State Management & Data Flow
- **React Hooks**: useState, useEffect for local state management
- **Additional State Management**: Context API, Redux, Zustand, or other state management libraries as needed
- **Drag and Drop**: Optional library for drag and drop functionality (if needed)
- **HTTP Client**: Axios or Fetch for API communication

### Testing Framework
- **E2E Testing**: Cypress, Playwright, or similar for end-to-end testing
- **Unit Testing**: Jest, Vitest, or similar for unit testing
- **Component Testing**: React Testing Library for component testing utilities
...
- **Component Naming**: Use PascalCase for React components (e.g., `EntityCard`, `ItemDetails`, `Dashboard`)
- **Variable Naming**: Use camelCase for variables and functions (e.g., `entityId`, `handleSubmit`, `fetchItems`)
- **Constants Naming**: Use UPPER_SNAKE_CASE for constants (e.g., `MAX_ITEMS_PER_PAGE`, `API_BASE_URL`)
- **Type/Interface Naming**: Use PascalCase for types and interfaces (e.g., `EntityData`, `ItemProps`, `IEntityService`)
- **File Naming**: Use PascalCase for component files (e.g., `EntityCard.tsx`, `ItemDetails.tsx`) and camelCase for utility files (e.g., `entityService.js`, `apiUtils.ts`)
- **CSS Class Naming**: Use kebab-case for CSS classes (e.g., `entity-card`, `item-details`)
- **Hook Naming**: Use camelCase starting with "use" prefix (e.g., `useEntity`, `useItemData`, `useFormValidation`)
...
const EntityCard: React.FC<EntityCardProps> = ({ entity, index, onClick }) => {
    const [isLoading, setIsLoading] = useState(false);
    
    // Handle entity card click event
...
const TarjetaEntidad: React.FC<PropsTarjetaEntidad> = ({ entidad, indice, alHacerClic }) => {
    const [estaCargando, setEstaCargando] = useState(false);
    
    // Manejar evento de clic en la tarjeta de entidad
    const manejarClicTarjeta = () => {
        alHacerClic(entidad);
    };
    
    return (
        <div className="tarjeta-entidad" onClick={manejarClicTarjeta}>
            {/* JSX del componente */}
        </div>
    );
};
...
// Avoid: Non-English messages
    ...
    setError('No se pudieron cargar las entidades. Por favor, inténtelo de nuevo más tarde.');
}
```
...
export const servicioEntidades = {
    obtenerTodasLasEntidades: async () => {
        try {
            const respuesta = await axios.get(`${API_BASE_URL}/entities`);
...
- Use **UI component library** (React Bootstrap, Material-UI components, etc.) instead of plain HTML/CSS
- **Import CSS framework** in the main App component
- Follow **responsive grid system** (Container, Row, Col or equivalent)

```javascript
import { Container, Row, Col, Card, Button, Form, Alert } from 'react-bootstrap';
// Or your chosen UI library components
```
...
- **Organize tests by feature** (entities.cy.ts, items.cy.ts, etc.)
- **Include API testing** alongside UI testing

```typescript
describe('Items API - Update', () => {
    beforeEach(() => {
        cy.window().then((win) => {
...
This document serves as the foundation for maintaining code quality and consistency across frontend applications. All team members should follow these practices to ensure a maintainable and scalable codebase.
